...
const {
  processCalls
} = require('./processCalls.js');

const processPollTimes = (dbClient, done, company_name, companiesObj, pollTimesQueue, participantsArray, transcribe, cb) => {
  ...
  waterfall([
      ...
      function (arrOfCalls, callback) {
        if(arrOfCalls.result !== 'fail' && arrOfCalls.length > 0) {
          processCalls(dbClient, done, company_name, companiesObj, arrOfCalls, participantsArray, transcribe, callback);
        } else {
    ...
    function (err) {
      if(err) {
        ...
        processPollTimes(dbClient, done, company_name, companiesObj, pollTimesQueue, participantsArray, transcribe, cb);
      } else {
        cb(null);
      }
    });
