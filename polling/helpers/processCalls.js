...
const {
  checkFilesTable,
  checkCallsTable,
  checkTranscriptionsTable,
  checkParticipantsTable
} = require('../db/checkTables.js');
const {
  insertIntoTranscriptionsTable
} = require('../db/insertData.js');
...
const processCalls = (dbClient, done, company_name, companiesObj, arrOfCalls, participantsArray, transcribe, cb) => {
  ...
  waterfall([
      ...
      function (callback) {
        ...
        checkFilesTable(dbClient, thisCall, done, (err, file_id, command) => {
          if(err) {
            ...
            var fname = thisCall.filePath = process.env.SAVE_AUDIO_PATH + file_id + '.wav';
            if(command === 'INSERT') {
      ...
      function (callback) {
        ...
        if(thisCall.filePath && thisCall.filePath.length) {
          checkTranscriptionsTable(dbClient, thisCall, done, (status, id) => {
            if(!id) {
          ...
          console.log('no file for call:', thisCall);
          callback();
        }
      }
    ],
    function (err) {
      if(err) {
        ...
        processCalls(dbClient, done, company_name, companiesObj, arrOfCalls, participantsArray, transcribe, cb);
      } else {
        cb(null, 'success');
      }
    });
