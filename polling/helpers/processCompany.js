...
const processPollTimes = require('./processPollTimes.js')
  .processPollTimes;
...
const processCompany = (dbClient, done, companyNamesQueue, companiesObj, startPollTime, participantsArray, transcribe, cb) => {
  ...
  waterfall([
      function (callback) {
        ...
        checkCompaniesTable(dbClient, {
          company_name: company_name
        }, done, (err, company_id) => {
          if(err) {
            callback(err);
          } else {
            companiesObj[company_name] = {
    ...
    function (err) {
      if(err) {
        ...
        processCompany(dbClient, done, companyNamesQueue, companiesObj, startPollTime, participantsArray, transcribe, cb);
      } else {
