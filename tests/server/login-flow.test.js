...
tape('test the login user flow for a successful api response from IPC', (t) => {
  t.plan(7);
  server.inject({
    method: 'POST',
    url: '/login',
    ...
    expected = '/dashboard';
    t.deepEqual(actual, expected, 'redirects to dashboard route');
  });
});
